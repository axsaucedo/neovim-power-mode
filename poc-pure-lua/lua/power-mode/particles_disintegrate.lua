...
-- Tumble animation frames (simulates rotation)
local tumble_frames = { "╱", "─", "╲", "│" }
...
function M.spawn(row, col)
  ...
  for dy = -1, 1 do
    local line_idx = dy + 1 + 1  -- 1-indexed into lines table
    local line = lines[line_idx]
    if not line then goto next_row end

    for dx = -3, 3 do
      if dx == 0 and dy == 0 then goto next_col end  -- skip cursor position
...
function M.update(dt)
  ...
  while i <= #active do
    ...
    p.x = p.x + p.vx * dt
    p.y = p.y + p.vy * dt
    -- Gravity — shattered pieces fall
    p.vy = p.vy + 0.2 * dt * 60
    p.vx = p.vx * 0.96
    p.vy = p.vy * 0.96
    p.lifetime = p.lifetime - dt * 1000

    -- Tumble effect: cycle through rotation frames
