...
- **Every subsystem exposes a cheap `is_idle()`.** The engine's per-tick fast-path returns before any update/merge/render when all of particles/fire/fire_wall/combo are idle. Any new subsystem must participate, or idle CPU regresses.
